...
parser = argparse.ArgumentParser(
    description="Process battery data from Car Scanner and calculate internal resistance statistics."
)
...
parser.add_argument(
    ...
    type=float,
    required=True,
    help="Nominaler/gesunder Akku Widerstand in mOhm",
)
parser.add_argument(
    "--cable_bms_ir",
    type=float,
    default=0.2,
    help="Kabel und BMS Widerstand (default: 0.2 mOhm)",
)
parser.add_argument(
    "--cells", type=float, default=96, required=True, help="Anzahl Zellen des Akkus"
)
...
soc_vs_resting_voltage = resting_data.groupby("Battery State of Charge")[
    "Battery voltage"
].mean()
soc_vs_resting_voltage_smoothed = savgol_filter(
    soc_vs_resting_voltage, window_length=11, polyorder=2
)
soc_vs_resting_voltage_interp = np.interp(
    df["Battery State of Charge"],
    soc_vs_resting_voltage.index,
    soc_vs_resting_voltage_smoothed,
)
...
ax1.plot(
    df["SECONDS"],
    soc_vs_resting_voltage_interp,
    color="red",
    label="Interpolierte Ruhespannung",
)
...
fig.tight_layout()
plt.title(
    "Interpolierte Ruhespannung und zugehörige Ströme (nur Entladung) über der Zeit"
)
plt.show()
...
load_data["Resting voltage"] = np.interp(
    load_data["Battery State of Charge"],
    soc_vs_resting_voltage.index,
    soc_vs_resting_voltage_smoothed,
)
load_data["Internal resistance"] = (
    load_data["Resting voltage"] - load_data["Battery voltage"]
) / load_data["Battery current"]
load_data["Internal resistance (mOhm)"] = (
    np.abs(load_data["Internal resistance"]) / args.cells * 1000
)  # Umrechnung in mOhm und Normalisierung auf die Zellenzahl
...
probabilities = load_data.groupby("Discharge rate bin")[
    "Internal resistance (mOhm)"
].apply(lambda x: np.sum(x < threshold) / len(x))
...
for name, group in load_data.groupby("Discharge rate bin"):
    ...
    exceeding_percentage = (
        np.sum(sorted_resistances > nominal_ir) / len(sorted_resistances) * 100
    )
...
plt.figure(figsize=(12, 8))
sns.violinplot(
    x="Discharge rate bin",
    y="Internal resistance (mOhm)",
    data=load_data,
    inner="quartile",
)
...
plt.ylabel("Internal resistance (mOhm)")
plt.title(
    "Violin plot of Internal Resistances by Discharge Rate\n with Probabilities < "
    + str(threshold)
    + " mOhm"
)
plt.grid(True)
...
variance_by_current = load_data.groupby("Discharge rate")[
    "Internal resistance (mOhm)"
].var()

plt.figure(figsize=(12, 8))
plt.plot(
    variance_by_current.index, variance_by_current, marker="o", linestyle="-", color="b"
)
plt.xlabel("Discharge rate (A)")
...
print(
    f"Die Wahrscheinlichkeit, dass der Innenwiderstand kleiner als {threshold} mOhm ist:"
)
print(probabilities)
